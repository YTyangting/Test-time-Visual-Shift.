...
def reset_cfg(cfg, args):
    ...
    if args.backbone:
        cfg.MODER.BACKBONE.NAME = args.backbone
...
def extend_cfg(cfg):
    ...
    cfg.TRAINER.TVS = CN()
    cfg.TRAINER.TVS.N_CTX_VISION= 2  # number of context vectors
    cfg.TRAINER.TVS.N_CTX_TEXT= 4
    cfg.TRAINER.TVS.CTX_INIT="a photo of a" 
    cfg.TRAINER.TVS.CLASS_TOKEN_POSITION = "end"  # 'm
    cfg.TRAINER.TVS.PREC = "amp"  # fp16, fp32, amp
    
    cfg.DATASET.SUBSAMPLE_CLASSES = "all"  # all, base or new
    cfg.DATASET.TPT = 'I'
    cfg.DATASET.VARIANT = 'Worlds'  # Added for PUG dataset variants

    cfg.DATASET.ENABLE = True
    cfg.DATASET.SHOTCAP = 3
    cfg.DATASET.ALPHA = 6.0
    cfg.DATASET.BETA = 5.0 
    cfg.DATASET.LR = 5e-2
    cfg.DATASET.RATE = 0.5
    cfg.DATASET.SELECT=1e-1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device", type=int, default=1, help="only positive value enables a fixed seed"
    )
    parser.add_argument(
